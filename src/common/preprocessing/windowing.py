...
def window_signal(
    x, fs, window_ms=200, hop_ms=100, labels=None, timestamps_ms=None
    ...
    for start in range(0, n_samples - win_len + 1, hop_len):
        ...
        if labels is not None and timestamps_ms is not None:
            ...
            for (start_i, end_i), label_i in zip(timestamps_ms, labels):
                ...
                if overlap > 0:
                    overlap_durations[label_i] = (
                        overlap_durations.get(label_i, 0) + overlap
                    )
...
def window_segment_multichannel(
    ...
    if segments.ndim != 2:
        raise ValueError("segments must have shape (samples, channels)")
    ...
    for ch in range(n_channels):
        ...
        elif windows_ch.shape[0] != n_windows:
            raise ValueError("Mismatch in window count between channels")

        all_ch_windows.append(windows_ch)

    # shape: (N, W, C)
    all_ch_windows = np.stack(all_ch_windows, axis=2).astype(np.float32)
    return all_ch_windows


def window_signal_np(
    x, fs, window_ms=200, hop_ms=100, labels=None
